...
@Component({
  standalone: true,
  imports: [NgIf, NgFor, RouterLink, PageHeaderComponent, HasRoleDirective],
  template: `
    ...
    <div class="card card-pad stack">
      <table class="table" *ngIf="filteredUsers().length > 0; else empty">
        <thead>
          <tr>
            <th>ID</th>
            <th>Username</th>
...
export class UserListPageComponent implements OnInit {
  ...
  readonly filteredUsers = computed(() => {
    ...
    return q ? this.users().filter((user) => `${user.id} ${user.username}`.toLowerCase().includes(q)) : this.users();
  });
